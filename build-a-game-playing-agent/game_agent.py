		...
		**********************************************************************
		...
		try: # there are legal moves available
			...
			if self.method == 'minimax':
				score, move = minimax(game, self.search_depth)
			elif self.method == 'alphabeta':
				score, move = alphabeta(game, self.search_depth)
			else:
				pass
			return move
		except Timeout:
		...
		if self.time_left() < self.TIMER_THRESHOLD:
			...
			for i in depth:
				...
				if maximizing_player == True:
					return max([(game.forecast_move(m),m) for m in legal_moves])
				else: #minimizing
					return min([(game.forecast_move(m),m) for m in legal_moves])
				maximizing_player = not maximizing_plater
		# TODO: finish this function!
		#raise NotImplementedError
...
>>>>>>> macbook 18-1-17 before pulling from repository
